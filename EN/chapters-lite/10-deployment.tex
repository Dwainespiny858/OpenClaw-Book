\chapter{Hands-On: Deploying and Using OpenClaw on Tencent Lighthouse Server}
...
This hands-on exercise uses a \textbf{Tencent Lighthouse Server}, configured as follows:
...
\hline
Server Public IP & \texttt{your-server-ip} \\
\hline
Username & ubuntu \\
\hline
OS & Ubuntu 24.04 LTS (Noble Numbat) \\
\hline
Kernel & Linux 6.8.0-51-generic (x86\_64) \\
\hline
CPU & Intel Xeon Platinum 8255C @ 2.50GHz $\times$ 2 cores \\
\hline
Memory & 2 GB \\
\hline
Disk & 40 GB SSD (17 GB used, 21 GB free) \\
\hline
Node.js Version & v22.22.1 \\
\hline
OpenClaw Version & 2026.3.2 \\
\hline
Key File & \texttt{\textasciitilde{}/.ssh/<your-key>.pem} \\
\hline
\end{tabular}
\end{table}
...
Connect to the Tencent Lighthouse server from your local Mac/Linux terminal:

\begin{lstlisting}[language=bash]
# Private key permissions must be 600, otherwise SSH will refuse
chmod 600 ~/.ssh/<your-key>.pem

# Connect to Tencent Lighthouse server
...
After a successful connection, you will see the Ubuntu 24.04 LTS welcome message:
...
\begin{itemize}
  \item \textbf{Node.js 22+}(installer script auto-detects and installs)
  \item macOS, Linux, or Windows via WSL2
  \item \texttt{pnpm} only needed when building from source
\end{itemize}

\subsubsection{Method 1: Installer Script (Recommended)}

The installer script handles Node.js detection, installation, and onboarding in one step:
...
\begin{enumerate}
  ...
  \item Ensure Node.js 22+ is installed (via NodeSource on Linux)
  \item Ensure Git is installed
  \item Install OpenClaw globally via npm
  \item 运行新手引导（onboarding）
\end{enumerate}

To skip onboarding (for automated deployment):

\begin{lstlisting}[language=bash]
curl -fsSL https://openclaw.ai/install.sh | bash -s -- --no-onboard
\end{lstlisting}

\subsubsection{Method 2: Global npm Install (Manual)}
...
\subsubsection{Method 3: Build from Source}

For scenarios requiring custom modifications or development:
...
\subsubsection{Low-Spec Server Optimization}

腾讯轻量云的 2核2GB 属于较低配置，可以通过以下环境变量优化 OpenClaw 的启动速度：

\begin{lstlisting}[language=bash]
# Enable Node.js compile cache, speed up repeated starts
export NODE_COMPILE_CACHE=/var/tmp/openclaw-compile-cache
mkdir -p /var/tmp/openclaw-compile-cache

# Disable auto-restart to reduce startup overhead
export OPENCLAW_NO_RESPAWN=1

# Write to ~/.bashrc for permanent effect
...
After installation, use OpenClaw via the \texttt{openclaw} command.
...
\subsection{Configuration Method}
...
# Set model (model ID must match the API-supported name)
...
# Test Agent (send a message, check reply)
ubuntu@VM-4-11-ubuntu:~$ openclaw agent --agent main \
  --message 'Hello, briefly introduce yourself in under 50 words'
I am your AI assistant OpenClaw. I can help you process documents, search information,
manage files, and more. Concise, efficient, always at your service.
\end{lstlisting}
...
\section{\texttt{\textasciitilde{}/.openclaw/} Directory Structure Explained}
...
\begin{lstlisting}
~/.openclaw/
+-- openclaw.json              # Main config (API keys, models, gateway, channels, plugins, etc.)
+-- openclaw.json.bak          # Config auto-backup (up to 5 copies)
+-- openclaw.json.bak.1
+-- openclaw.json.bak.2
+-- openclaw.json.bak.3
+-- openclaw.json.bak.4
+-- update-check.json          # Version update check records
|
+-- agents/                    # Agent registry (one subdirectory per Agent)
|   +-- main/                  # Default Agent "main"
|   |   +-- agent/
|   |   |   +-- models.json    # Runtime model info snapshot
|   |   +-- sessions/
|   |       +-- sessions.json  # Session index (29 sessions)
|   |       +-- <uuid>.jsonl   # Message history for specific session
|   +-- coder/                 # Coder Agent (independent workspace)
|   +-- work/                  # Work Agent
|
...
|   +-- jobs.json              # Task registry (e.g., drink water reminder)
...
|   +-- pending.json
|
+-- extensions/                # External plugin directory
|   +-- feishu/                # Feishu plugin (@openclaw/feishu v2026.3.7)
|
+-- feishu/                    # Feishu channel runtime data
|   +-- dedup/                 # Message dedup cache
|
...
|
+-- logs/                      # Runtime logs
|   +-- config-audit.jsonl     # Config change audit log
|
+-- memory/                    # Memory storage
|   +-- main.sqlite            # Default Agent SQLite memory DB
|
...
|   +-- USER.md                # User profile (recorded: Luck, Asia/Shanghai)
|   +-- BOOTSTRAP.md           # First-start bootstrap
|   +-- HEARTBEAT.md           # Heartbeat task definition
|   +-- TOOLS.md               # Local tools memo
|   +-- memory/                # Daily memory logs
|   +-- skills/
|       +-- xiaohongshu-mcp/   # Installed: Xiaohongshu automation Skill
|
+-- workspace-coder/           # coder Agent 独立工作空间
    +-- .git/
    +-- .openclaw/
    +-- SOUL.md                # Coder's independent persona
    +-- memory/
\end{lstlisting}
...
\subsubsection{openclaw.json.bak* --- Config Auto-Backup}
...
\subsubsection{update-check.json --- Version Check Records}
...
\subsection{\texttt{cron/} --- Scheduled Tasks}
...
+-- skills/          # 通过 ClawHub 安装的工作空间级 Skills
    +-- <skill-name>/
        +-- .clawhub/origin.json   # Installation source info
        +-- _meta.json             # Skill metadata
        +-- SKILL.md               # Skill definition file
        +-- scripts/               # Skill bundled scripts (optional)
\end{lstlisting}

\subsubsection{SOUL.md --- Core Persona Definition}
...
\subsubsection{AGENTS.md --- Agent Workflow Definition}
...
\subsubsection{IDENTITY.md --- Agent Identity Information}
...
\subsubsection{BOOTSTRAP.md --- First-Start Bootstrap}
...
\subsubsection{HEARTBEAT.md --- Heartbeat Tasks}
...
+-- _meta.json             # Skill metadata（作者ID、slug、版本、发布时间）
+-- SKILL.md               # Skill definition file（名称、描述、使用指南）
